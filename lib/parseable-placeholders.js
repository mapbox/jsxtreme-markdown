...
module.exports = (input, delimiters, escapeDelimiter) => {
  ...
  for (index = 0; index < length; index++) {
    ...
    const match = balancedMatch(
      startDelimiter,
      endDelimiter,
      input.slice(index)
    );
    resumeAt = index + match.end + endDelimiter.length - 1;

    // We can count on the match's starting position to be unique,
    // so usable as a numeric id.
    const matchId = index + match.start;
    const matchValue = match.body.trim();

    // Distinguish between expressions and tags,
    // and inline- and block-level tags.
    const isTag = /^</.test(matchValue);
    let isInline = !isTag;
    if (isTag) {
      const lineBefore = endsWithNewline(input.slice(0, index));
      const lineAfter = startsWithNewline(
        input.slice(index + match.end + endDelimiter.length)
      );
      ...
      if (isInline) {
        ...
        if (blockElementsSet.has(tagName)) {
          const errorIndex = !lineBefore
            ? match.start + index
            : match.end + index + endDelimiter.length - 1;
