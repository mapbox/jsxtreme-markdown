...
module.exports = (input, delimiters, escapeDelimiter) => {
  const placeholders = {};
  const transformNextPlaceholder = (str, index, offset) => {
    const searchStr = str.slice(index);
    const match = balancedMatch(delimiters[0], delimiters[1], searchStr);
    if (!match) return str;

    if (searchStr[match.start - 1] === escapeDelimiter) {
      const nextStr =
        searchStr.slice(0, match.start - 1) + searchStr.slice(match.start);

      return transformNextPlaceholder(
        str.slice(0, index) + nextStr,
        index + match.end - 1,
        offset - 1
      );
    }
    ...
    const nextStr = [
      str.slice(0, index) + match.pre,
      representation,
      match.post
    ].join('');
    const nextSearchIndex = index + match.start + representation.length;
    return transformNextPlaceholder(
      nextStr,
      nextSearchIndex,
      input.length - nextStr.length
    );
  };

  const text = transformNextPlaceholder(input, 0, 0);
  return {
    placeholders,
    text
  };
