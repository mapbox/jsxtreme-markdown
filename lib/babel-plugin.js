...
module.exports = babel => {
  ...
  const taggedTemplateExpressionVisitor = path => {
    ...
    if (quasi.expressions.length !== 0) {
      throw path
        ...
        .buildCodeFrameError(
          'Placeholders are not supported inside md-react-transformer tagged template literals'
        );
    }

    const markdown = quasi.quasis.map(quasi => quasi.value.cooked).join('');
    const jsx = mdToJsx(markdown);
